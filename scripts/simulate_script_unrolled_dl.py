...
parser.add_argument("--milestones", type=list, default=[150]) # --milestones 400 450 475 
...
#   logmeanexp = np.load(processing_params['logmeanexp'][params['dataset_index']])
  ...
  data = xDzDataset(sim_params)
  X = data.x.clone().detach().cpu().numpy()
  D = data.D.clone()
  Z = data.z.clone()
  
  # INITIALIZING WITH RANDOM DICTIONARY FROM INPUT MATRIX X
  index_dict = torch.randint(low=0, high=X.shape[0], size =(params['num_modules'], ))
  tens_X = torch.squeeze(torch.tensor(X, dtype=torch.float32) )
  print("modules", params['num_modules'])
  print("tensX", tens_X.shape)
  print("indices",index_dict)
  W_init = torch.t(tens_X[index_dict,])  
  
  np.save(f"{tensorboardx_path}/true_dictionary.npy", data.D.cpu().numpy())
  np.save( f"{tensorboardx_path}/true_code.npy", np.squeeze(data.z, axis=2).cpu().numpy())
  np.save(f"{tensorboardx_path}/true_data_X.npy", np.squeeze(data.x, axis=2).cpu().numpy())
  
  # INTIALIZING WITH TRUE DICTIONARY
  # W_init = D
...
#   sim_params["num_genes"] = X.shape[1] # number of genes to use as dictionary
#   sim_params["num_cells"] = X.shape[0] # number of cells to use in training, we're using all cells so that nothing goes into the validation set

  num_train = int(sim_params["num_cells"]  * params["train_val_split"])

  train_dataset = MyDataset(X)
#   train_dataset = MyDataset(np.expand_dims(X, axis=2))

  # make dataloader
  train_loader = torch.utils.data.DataLoader(
  train_dataset,
  shuffle=params["shuffle"], # MAKE SURE TO CHANGE
...
#   np.save( f"{tensorboardx_path}/code_no_learning.npy", code.detach().cpu()) 
